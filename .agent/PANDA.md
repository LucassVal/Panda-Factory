...
| [PF_RUST_REFERENCE.md](../docs/PF_RUST_REFERENCE.md)                             | Rust Agent  | MCP, GPU, Firebase Direct             |
| [PF_MEDUSA_REFERENCE.md](../docs/PF_MEDUSA_REFERENCE.md)                         | Plugins     | GitHub Hooks, VMs, Store              |
...
## 📐 PADRÕES ARQUITETURAIS (Constituição)

### §7.6 Fault Isolation Pattern

> **REGRA:** Nenhum hook pode `throw error` - falhas devem retornar graciosamente.
...
- Timeout obrigatório: 30 segundos por chamada
- Erros reportados via `TM.reportError()`
- Status marcado como "error" no TentacleMonitor
- Outros hooks continuam funcionando normalmente

### §7.7 Error Registry Protocol

> **REGRA:** Todos os erros de hooks devem ser registrados para visibilidade de agentes.

| Método                        | Uso                                |
| ----------------------------- | ---------------------------------- |
| `TM.reportError()`            | Registrar erro de hook             |
...
| `Panda.emit('monitor:error')` | Evento real-time para listeners    |
...
- `isLoading` state obrigatório
- `error` state com `clearError()` method
- `useCallback` para memoization de funções
- Timeout para operações assíncronas

### §12.3 Tentacle Parent/Child Architecture

> **REGRA:** Todo módulo externo deve seguir padrão Parent → Child.

```text
   Parent (Agregador)
   ├── registerChild(name, api)
   ├── _wrapChild(name, api)  ← FAULT ISOLATION AQUI
   └── api.{childName}
```

**Workflow:**

1. Child se registra no Parent via `registerChild()`
2. Parent wraps Child com fault isolation
3. Erros não propagam para outros children
4. TentacleMonitor recebe erro via `reportError()`
...
## 6. CONSTITUIÇÃO (14 ARTIGOS)

> **ATUALIZADO:** v1.1 (25/Jan/2026) - Artigos 13+14, Split 52/25/15/5/3
...
| 13  | **Developer First** | 10% off (2.5x→2.25x) se sustentável      | `dev_discount`         |
| 14  | **Buyback Auto**    | Inflação >5%/30d → 70% burn, 30% reserva | `auto_buyback`         |

### 6.1 Artigo 13: Developer First

- **Desconto:** 10% (2.5x → 2.25x)
- **Gatilhos:** MAU≥1000 + Health≥80% + Runway≥12mo **OU** deflação >2%
- **Split protegido:** 55/22/15/5/3

### 6.2 Artigo 14: Buyback Automático

- **Gatilho:** Inflação >5% por 30 dias
- **Limites:** 10%/mês, 2% desconto
- **Split:** 70% burn, 30% reserva
- **Dual-Mode:** OFF_CHAIN (PIX) → ON_CHAIN (Solana futuro)

### 6.3 Split Atual: 52/25/15/5/3

| Destino         | %   | Nota     |
| --------------- | --- | -------- |
| Dev/Host        | 52% | -        |
| Fundo Incentivo | 25% | MIN ≥25% |
| Panda Ops       | 15% | MIN ≥15% |
| Founder         | 5%  | Fixo     |
| Gateway/GAS     | 3%  | -        |
...
## 13.5 FOUNDER DASHBOARD (MONITORS) 📊

> **Objetivo:** Visibilidade total do ecossistema para o Founder (Camada 1).
> **Referência:** [PF_TOKENOMICS_REFERENCE.md §15](../docs/PF_TOKENOMICS_REFERENCE.md#15-founder-dashboard-monitors)
...
### Arquitetura de Capacidade (Free Tier)

```text
📊 CAPACIDADE ESTIMADA (Client-Side First)

├── Usuários Cadastrados: ~100,000+
├── Usuários Ativos Simultâneos: ~10,000 (Firebase RTDB limit)
├── Chamadas GAS/dia: ~50,000 (só billing/auth)
├── IA (Gemini Compartilhada): ~400 usuários/dia
└── Com Rust Agent: ∞ (processamento local)
```

### Gemini API Compartilhada

| Nível       | Quem     | Modelo      | Quota/dia   | Fonte           |
| ----------- | -------- | ----------- | ----------- | --------------- |
| User (3)    | Usuários | Flash 3.0   | 300k tokens | Conta Founder   |
| Dev (2)     | Devs     | Flash + Pro | 400k tokens | Conta Founder   |
| Founder (1) | Lucas    | Todos       | ∞           | Própria         |
